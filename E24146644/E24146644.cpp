...
vector<Move> getMoves(const string& color, bool mustEat) {
    ...
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
            if (board[i][j] < myMin || board[i][j] > myMax) continue;  //搜索到自己的棋子才開始判斷下面的走法

            for (int d = 0; d < 6; d++) { // 6 個方向
                for (int step = 1; step < 9; step++) { // 最多走 8 步（整條線）
                    int ni = i + dx[d] * step;
                    int nj = j + dy[d] * step;
                    ...
                    if (board[ni][nj] != 0) {
                        // Eat
                        m = {'E', i, j, ni, nj};    //掃描到了有東西，預設為eat邏輯
                        if (isValid(m, color, mustEat)) res.push_back(m);  //把m傳入isValid 做判斷  

                        // Stack
                        if (!mustEat) {  //step2 中，的堆子情況。吃子已經被前面運行過了
                    ...
                    /*if (!mustEat) {
                        m = {'S', i, j, ni, nj};
                        if (isValid(m, color, false)) res.push_back(m);
                    }*/ //這段是移到沒有旗子的空位。是錯誤的AI給我生的bug。必須移動到其他棋子上。
                }
            }
        }
    }
...
// ===== 座標轉換 =====
Move transformMove(Move m) {    //pass是0000
...
void readBoard(const string& path) {
    ...
    for(int i=0;i<9;i++){ //9行
        ...
        for(int j=0;j<9;j++){ //9個
            string val;
            getline(ss,val,','); //以一個，為分隔符號。切割數據//遇到逗號就下一步stoi 轉成int 存入board陣列
            board[i][j]=stoi(val);
        }
    }
}
...
int minimax(int depth, int alpha, int beta, bool isMaximizing, const string& currentColor, int step, int b[9][9], int l[9][9], const string& myColor, Timer& tmr, double time_limit) {
    ...
    if (moves.empty()) {
        return isMaximizing ? (-1000000 - depth) : (1000000 + depth); //移動度之死，"好死不如賴活著"
    }
