...
//  orderMoves模塊
//==============================================================
// 這裡的排序邏輯非常簡單，主要是讓吃子優先於疊子，其他細節不多考慮
// 輔助結構，用來綁定著法與其猜測分數
struct MoveScore {
    Move move;
    int guessScore;
...
void orderMoves(vector<Move>& moves, int b[9][9], int l[9][9], string currentColor, int step) {
    ...
    for (const auto& m : moves) {
        ...
        if (step == 1) {
            ...
            // 策略：傾向用較矮的棋子去發動攻擊，保留己方高塔的機動性與生存率。
            // 攻擊者高度越高，稍微扣一點分。
            score -= l[fx][fy] * 10; 

        } else if (step == 2) {
            // 【第二步：吃子或強化】
            // 策略：不管終點是敵方還是己方，參與互動的目標高度越高，通常影響力越大。
            // 由於第二步的強制性較低，給予中等的權重。
            score += l[tx][ty] * 50; 
        }
    ...
    // 3. 根據 guessScore 由大到小降冪排序 (使用 <algorithm> 的 std::sort)
...
int minimax(int depth, int alpha, int beta, bool isMaximizing, string currentColor, int step, int b[9][9], int l[9][9], string myColor) {
    ...
    orderMoves(moves, b, l, currentColor, step); // 排序未來推演的著法
...
void findBestMove(string myColor, int currentRound) {
    ...
    orderMoves(firstMoves, board, layer, myColor, 1); // 排序第一動
    ...
    // 2. 取得所有合法的第二步 (因為全域 board 已經更新了，所以會取到正確的第二步)
        ...
        orderMoves(secondMoves, board, layer, myColor, 2); // 排序第二動
