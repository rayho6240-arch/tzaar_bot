...
// 建議的輔助 Penalty 數值，請確保你的 getPenalty 至少要這麼重
int getPenalty(int count) {
    if (count <= 0) return 1000000; // 滅族
    if (count == 1) return 15000;   // 極度危險！絕對不要變成 1
    if (count == 2) return 2000;    // 警告
    if (count == 3) return 500;     // 輕微壓力
    return 0;
}
